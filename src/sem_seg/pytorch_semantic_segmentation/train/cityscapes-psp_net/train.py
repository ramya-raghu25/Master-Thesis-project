...
DESCRIPTION:     Python script for training all semantic segmentation networks
Date: 20.10.2019
...
from pjval_ml.OSR.GAN.src.sem_seg.pytorch_semantic_segmentation.utils import joint_transforms as joint_transforms
import pjval_ml.OSR.GAN.src.sem_seg.pytorch_semantic_segmentation.utils.transforms as extended_transforms
from pjval_ml.OSR.GAN.src.sem_seg.pytorch_semantic_segmentation.datasets import cityscapes
from pjval_ml.OSR.GAN.src.sem_seg.pytorch_semantic_segmentation.models import *
from pjval_ml.OSR.GAN.src.sem_seg.pytorch_semantic_segmentation.utils import check_mkdir, evaluate, AverageMeter, CrossEntropyLoss2d

ckpt_path = Path(PJVAL_SHARE, 'data','samples_osr_yolo','GAN','exp','train_result')
exp_name = 'cityscapes(fine)-bayseg_net1'
writer = SummaryWriter(os.path.join(ckpt_path, 'exp', exp_name))
print(os.path.join(ckpt_path, 'exp', exp_name))

args = {
    'train_batch_size': 2,
    'lr': 1e-2 / sqrt(16 / 2),
    'lr_decay': 0.9,
    'max_iter': 8500,  #2532 #9e4
    'longer_size': 2048,
    'crop_size': 713,
    'stride_rate': 2 / 3.,
    'weight_decay': 1e-4,
...
def main():
    ...
    #net = SegNetBayes(num_classes=cityscapes.num_classes)
    ...
    open(Path(ckpt_path, exp_name, str(exp_name)+'.txt'), 'w').write(str(args) + '\n\n')
    ...
    while True:
        ...
        for i, data in enumerate(train_loader):
            ...
            for inputs_slice, gts_slice in zip(inputs, gts):
                ...
                train_main_loss.update(main_loss.data, slice_batch_pixel_size) #data[0]
                train_aux_loss.update(aux_loss.data, slice_batch_pixel_size)  #data[0]
...
def validate(val_loader, net, criterion, optimizer, epoch, iter_num, train_args, visualize):
    ...
    for vi, data in enumerate(val_loader):
        ...
        for input_slice, gt_slice, info in zip(input, gt, slices_info):
            ...
            val_loss.update(criterion(output_slice, gt_slice).data, slice_batch_pixel_size)  #data[0]
