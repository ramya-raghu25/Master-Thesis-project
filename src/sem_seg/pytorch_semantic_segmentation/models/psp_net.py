...
DESCRIPTION:     Python script for PSPNet
Date: 20.10.2019
...
def _resnet(arch, block, layers, pretrained, progress, **kwargs):
    ...
    if pretrained:
        ...
        state_dict = torch.load(os.path.join(PJVAL_SHARE, 'data', 'samples_osr_yolo', 'GAN', 'dataset',
                                             'Weights', 'resnet101-5d3b4d8f.pth'))
        model.load_state_dict(state_dict)
        print("loaded resnet101-5d3b4d8f")
    return model
...
class PSPNet(nn.Module):
    def __init__(self, num_classes, pretrained=True, use_aux=True):
        super().__init__()
        #resnet101_model = resnet101(pretrained=pretrained)   #while training
        self.use_aux = use_aux
        #print("resnet 101")
        resnet = models.resnet101(pretrained=pretrained)
        #print("resnet",resnet)
        #resnet = resnet101_model
